...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'Korbinian Sollner';
    var email     = grunt.option('email')  || 'ksollner@feinarbyte.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new probemon name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/probemon.png')) {
            fs.renameSync(__dirname + '/admin/probemon.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/probemon.html')) {
            fs.renameSync(__dirname + '/widgets/probemon.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/probemon/js/probemon.js')) {
            fs.renameSync(__dirname + '/widgets/probemon/js/probemon.js', __dirname + '/widgets/probemon/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/probemon')) {
            fs.renameSync(__dirname + '/widgets/probemon',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Probemon/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Probemon'
                        },
                        {
                            match: /Korbinian Sollner/g,
                            replacement: author
                        },
                        {
                            match: /ksollner@feinarbyte.de/g,
                            replacement: email
                        }
                    ]
                },
